...
void ShaderProgram::setVector(const char* nom, const std::vector<GLfloat>& tableau ) const
{
    ...
    glUniform1fv( localisation, (GLsizei)(tableau.size()), &tableau[0] ); // Sûr ??
}

void ShaderProgram::setMat4x4( const char* nom, const glm::mat4& matrice ) const
{
    glUniformMatrix4fv( glGetUniformLocation( this->indice, nom ), 1, GL_FALSE, &matrice[0][0] );
}

void ShaderProgram::setMat3x3( const char* nom, const glm::mat3& matrice ) const
{
    glUniformMatrix3fv( glGetUniformLocation( this->indice, nom ), 1, GL_FALSE, &matrice[0][0] );
}
