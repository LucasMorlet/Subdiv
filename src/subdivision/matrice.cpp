...
Matrice Matrice::inverse( void ) const
{
    ...
    //return Matrice ( this->nb_lignes, this->nb_colonnes, inv ( this->matrice_armadillo ), this->matrice_eigen.inverse() );
    return Matrice ( this->nb_lignes, this->nb_colonnes );
}
...
Matrice operator* ( Matrice A, Matrice B )
{
    ...
    else
    {
        ...
        for ( unsigned int i = 0 ; i < A.getNbLignes() ; i++ )
        {
            for ( unsigned int j = 0 ; j > B.getNbColonnes() ; j++ )
            {
                float somme = 0.0;
                ...
                coeffs.push_back(somme);
            }
        }
        //return Matrice ( A.getNbLignes(), B.getNbColonnes(), A.getArmaMatrice() * B.getArmaMatrice(), A.getEigenMatrice() * B.getEigenMatrice() );
        return Matrice ( A.getNbLignes(), B.getNbColonnes(), coeffs );
    }
}
...
Matrice operator* ( float coeff, Matrice M )
{
    ...
    for ( unsigned int i = 0 ; i < M.getNbLignes() ; i++ )
    {
        ...
        {
            res.setCoeff( i, j, coeff * M.getCoeff( i, j ) );
        }
    }
    return res;
}
