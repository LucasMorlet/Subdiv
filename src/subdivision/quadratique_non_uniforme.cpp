#include "quadratique_non_uniforme.h"

Quadratique_non_uniforme::Quadratique_non_uniforme( void )
{
    this->valence_ordinaire = 4;
    this->nom = QString ( "Quadratique non-uniforme" );
}
QString Quadratique_non_uniforme::getTES( void ) const
{
    return "quadratique_non_uniforme";
}

void tri_patch_2 ( std::vector < GLuint >& faces_adjacentes, std::vector < GLuint >& indices_faces )
{
...
void Quadratique_non_uniforme::faces2patches( Objet& obj ) const
{
    std::vector < GLfloat > lesSommets  = obj.getSommets();
    std::vector < GLuint  > indices     = obj.getIndices();

    // On fait un coup de vrai Doo-Sabin pour avoir un maillage qui respecte les conditions de la méthode => chaque sommet de valence 4
    ...
    for ( unsigned int i = 0 ; i < lesSommets.size() / 3 ; i++ )
    {
        ...
        {
            tri_patch_2 ( faces_adjacentes, indices );
...
// Les matrices
Matrice Quadratique_non_uniforme::T_0( unsigned int k ) const
{
    double V = 9.0 / 16.0;
    double E = 3.0 / 16.0;
    double F = 1.0 / 16.0;
...
Matrice Quadratique_non_uniforme::T_1( unsigned int k ) const
{
    double V = 9.0 / 16.0;
    double E = 3.0 / 16.0;
    double F = 1.0 / 16.0;
...
Matrice Quadratique_non_uniforme::T_2( unsigned int k ) const
{
    double V = 9.0 / 16.0;
    double E = 3.0 / 16.0;
    double F = 1.0 / 16.0;
...
Matrice Quadratique_non_uniforme::T_3( unsigned int k ) const
{
    double V = 9.0 / 16.0;
    double E = 3.0 / 16.0;
    double F = 1.0 / 16.0;
...
*/
Matrice Quadratique_non_uniforme::Identity( unsigned int k ) const
{
    return Matrice ( k+5 );
}
