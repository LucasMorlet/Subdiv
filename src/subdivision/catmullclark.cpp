...
void CatmullClark::faces2patches( Objet& obj ) const
{
    ...
    // On fait les corrections pour les textures
    std::vector < GLuint > indice_text = obj.getIndicesText();
    std::vector < GLfloat > sommets = obj.getSommets();             // Les sommets géométriques
    std::vector < GLfloat > sommets_text = obj.getSommetsText();    // Les sommets avec les coordonnées de textures
    for ( unsigned int i = 0 ; i < les_patchs.size()/2 ; i++ )
    {
        GLuint sommet_principal = indices_patchs.at ( les_patchs.at(2*i) );
        float x = sommets.at(3*sommet_principal);
        float y = sommets.at(3*sommet_principal+1);
        float z = sommets.at(3*sommet_principal+2);

        unsigned int j;
        for ( j = 0 ; j < 4 ; j++ )
        {
            GLuint sommet_test = indice_text.at(4*i+j);
            if ( x == sommets_text.at(3*sommet_test) && y == sommets_text.at(3*sommet_test+1) && z == sommets_text.at(3*sommet_test+2) )
            {
                break;
            }
        }

        new_indices.at(4*i  ) = indice_text.at(4*i+((j  )%4));
        new_indices.at(4*i+1) = indice_text.at(4*i+((j+1)%4));
        new_indices.at(4*i+2) = indice_text.at(4*i+((j+2)%4));
        new_indices.at(4*i+3) = indice_text.at(4*i+((j+3)%4));
    }

    // On modifie l'objet
    obj.setPatchs( les_patchs ); // Début + longueur
    obj.setNbPatchs( les_patchs.size() / 2 ); // Nb patch = nb faces
    obj.setIndicesPatchs( indices_patchs );
    obj.setFacesPatchs( new_indices );
    obj.setSommetsPatchs( obj.getSommets() ); // Les nouveaux sommets si on a fait des modifications
}
