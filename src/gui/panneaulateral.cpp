...
void PanneauLateral::creerPanneauShader ( void )
{
    ...
    this->labelTCS    = new QLabel("TCS");
    this->labelTES    = new QLabel("TES");
    ...
    // Les index de base
    this->menuVert->setCurrentIndex( this->menuVert->findText( "basique" ) );
    this->menuTCS ->setCurrentIndex( this->menuTCS ->findText( "quads" ) );
    this->menuTES ->setCurrentIndex( this->menuTES ->findText( "quads" ) );
    this->menuGeom->setCurrentIndex( this->menuGeom->findText( "basique" ) );
    this->menuFrag->setCurrentIndex( this->menuFrag->findText( "rouge" ) );
...
void PanneauLateral::changerSchema( int index )
{
    ...
    if ( index == Aucun )
    {
        ...
        if ( objCourant )
        {
            if ( objCourant->getTypeFace() == 4 )
            {
                this->menuTCS->setCurrentIndex( this->menuTCS->findText( "quads" ) );
                this->menuTES->setCurrentIndex( this->menuTES->findText( "quads" ) );
            }
            else if ( objCourant->getTypeFace() == 3 )
            {
    ...
    else
    {
        ...
        if ( index == Catmull_Clark || index == Doo_Sabin || index == Quadratique_enum )
        {
