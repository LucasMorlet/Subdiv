...
void PanneauScene::changerObjet( void )
{
    ...
    Objet* objCourant = this->scene->getObjetCourant();
    if ( objCourant )
    {
        const Materiau& materiau = objCourant->getMateriau();
...
void PanneauScene::changerLumiereCourante ( int n )
{
    ...
    const glm::vec3& position = lum.getPosition();
    this->spin_posX->setValue ( position.x );
    this->spin_posY->setValue ( position.y );
    this->spin_posZ->setValue ( position.z );

    const glm::vec3& couleur  = lum.getCouleur();
    this->spin_rouge->setValue( (unsigned int)(255 * couleur.r) );
    this->spin_vert->setValue ( (unsigned int)(255 * couleur.g) );
    this->spin_bleu->setValue ( (unsigned int)(255 * couleur.b) );
...
void PanneauScene::modificationLumiere( void )
{
    ...
    lum.setCouleur( (float)(this->spin_rouge->value()) / 255.0f, (float)(this->spin_vert->value()) / 255.0f, (float)(this->spin_bleu->value()) / 255.0f );
