...
void WidgetOpenGL::mouseMoveEvent( QMouseEvent* event )
{
    ...
    if ( objCourant )
    {
        ...
        this->scene->getCam().rotate( event->x() - this->startX, yy - this->startY, *objCourant );
        this->startX = event->x();
        this->startY = yy;
    }
}
...
void WidgetOpenGL::mouseReleaseEvent( QMouseEvent* event )
{
    ...
    if ( objCourant )
    {
        this->scene->getCam().rotate( event->x() - this->startX, this->height() - event->y() - this->startY, *objCourant );
    }
}
